...
test_whitelist_matches_github_host_case_insensitively() {
  echo -n "→ Testing whitelist matches GitHub host case-insensitively ... "
  rm -rf /tmp/repo && mkdir -p /tmp/repo
  cd /tmp/repo
  git init -q
  git config core.hooksPath /usr/src/app/hooks
  git config moonlight-commit.whitelistOrgs "myorg"
  echo "# mixed-case github host" > README.md
  git add README.md
  git commit --no-verify -q -m "init"
  git remote add origin https://GitHub.com/MyOrg/repo.git
  git checkout -q -b feature/mixed-case-github-host
  echo "change" >> README.md
  git add README.md

  if ! faketime -f "2025-04-30 10:00:00" git commit -q -m "mixed case github host" \
    >/tmp/moonlight-mixed-case-github-host.log 2>&1; then
    echo "❌"
    echo "Mixed-case GitHub origin host should match whitelist"; exit 1
  fi
...
test_whitelist_matches_github_ssh_scheme_case_insensitively() {
  echo -n "→ Testing whitelist matches GitHub SSH scheme case-insensitively ... "
  rm -rf /tmp/repo && mkdir -p /tmp/repo
  cd /tmp/repo
  git init -q
  git config core.hooksPath /usr/src/app/hooks
  git config moonlight-commit.whitelistOrgs "myorg"
  echo "# mixed-case github ssh scheme" > README.md
  git add README.md
  git commit --no-verify -q -m "init"
  git remote add origin SSH://git@GitHub.com/MyOrg/repo.git
  git checkout -q -b feature/mixed-case-github-ssh-scheme
  echo "change" >> README.md
  git add README.md

  if ! faketime -f "2025-04-30 10:00:00" git commit -q -m "mixed case github ssh scheme" \
    >/tmp/moonlight-mixed-case-github-ssh-scheme.log 2>&1; then
    echo "❌"
    echo "Mixed-case GitHub SSH scheme should match whitelist"; exit 1
  fi
...
test_whitelist_matches_github_ssh_port_22_origin() {
  echo -n "→ Testing whitelist matches GitHub SSH origins with explicit port 22 ... "
  rm -rf /tmp/repo && mkdir -p /tmp/repo
  cd /tmp/repo
  git init -q
  git config core.hooksPath /usr/src/app/hooks
  git config moonlight-commit.whitelistOrgs "myorg"
  echo "# github ssh port origin" > README.md
  git add README.md
  git commit --no-verify -q -m "init"
  git remote add origin ssh://git@github.com:22/MyOrg/repo.git
  git checkout -q -b feature/github-ssh-port-origin
  echo "change" >> README.md
  git add README.md

  if ! faketime -f "2025-04-30 10:00:00" git commit -q -m "github ssh port origin" \
    >/tmp/moonlight-github-ssh-port-origin.log 2>&1; then
    echo "❌"
    echo "GitHub SSH origin with explicit port 22 should match whitelist"; exit 1
  fi
...
test_whitelist_rejects_hidden_or_flag_like_github_repo_names() {
  echo -n "→ Testing whitelist rejects hidden or flag-like GitHub repo names ... "

  for repo in .repo -repo; do
    rm -rf /tmp/repo && mkdir -p /tmp/repo
    cd /tmp/repo
    git init -q
    git config core.hooksPath /usr/src/app/hooks
    git config moonlight-commit.whitelistOrgs "myorg"
    echo "# hidden github repo" > README.md
    git add README.md
    git commit --no-verify -q -m "init"
    git remote add origin "git@github.com:myorg/${repo}.git"
    git checkout -q -b "feature/reject-${repo#.}"
    echo "change" >> README.md
    git add README.md

    if faketime -f "2025-04-30 10:00:00" git commit -q -m "hidden or flag-like github repo" \
      >/tmp/moonlight-hidden-flag-repo.log 2>&1; then
      echo "❌"
      echo "GitHub origin with hidden or flag-like repo name should not match org whitelist"; exit 1
    fi

    grep -q "blocked between 9:00 and 17:00" /tmp/moonlight-hidden-flag-repo.log
  done
